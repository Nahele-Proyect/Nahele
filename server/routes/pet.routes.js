  ...
  Pet.create(req.body) //Create a pet
    ...
    .then((newPetId) =>
      User.findByIdAndUpdate(
        ...
        .populate("pets")
        .populate("calendar")
    ) //update the user with the pet, and populate the result
    .then((user) =>
      res.json({
        status: "ok",
        user,
...
router.post("/newScraped", (req, res, next) => {
  //Path to create a pet from the scraping, if the pet was already created, returns the existing one

  Pet.findOne(req.body) //Search for the dog existance in the DB
    .then((pet) => {
      if (pet) {
        res.json({
          status: "found",
...
router.get("/find/:id", (req, res, next) => {
  //Find a pet in the DB a dog by the mongo DB
...
router.delete("/delete/:id", (req, res, next) => {
  //Delete a pet in the Db by the mongo _id
...
router.post("/addRequest/:id", (req, res, next) => {
  //Add a request to the pet by the id given
  ...
  Pet.findByIdAndUpdate(req.params.id, {
    ...
    .then((pet) =>
      User.findById(req.user._id).populate("pets").populate("calendar")
    )
    .then((user) =>
      res.json({
        status: "ok",
